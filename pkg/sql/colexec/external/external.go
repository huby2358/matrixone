...
func scanCsvFile(ctx context.Context, param *ExternalParam, proc *process.Process, bat *batch.Batch) error {
	...
	if err := makeBatchRow(param, proc, bat); err != nil {
		return err
	}
	return nil
...
func makeBatchRow(param *ExternalParam, proc *process.Process, bat *batch.Batch) error {
	ctx := proc.Ctx
	...
	if err = initBatch(8192, proc, bat); err != nil {
		return err
	}
	for i := range bat.Vecs {
		bat.Vecs[i].SetLength(8192)
	}

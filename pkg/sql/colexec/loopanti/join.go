...
func (ctr *container) emptyProbe(ap *Argument, proc *process.Process, anal process.Analyze, isFirst bool, isLast bool, result *vm.CallResult) error {
	...
	for i, pos := range ap.Result {
		...
		// bat.Vecs[pos] = nil
		typ := *ap.ctr.bat.Vecs[pos].GetType()
		ctr.rbat.Vecs[i] = proc.GetVector(typ)
		if err := vector.GetUnionAllFunction(typ, proc.Mp())(ctr.rbat.Vecs[i], ap.ctr.bat.Vecs[pos]); err != nil {
			return err
		}
	}
...
func (ctr *container) probe(ap *Argument, proc *process.Process, anal process.Analyze, isFirst bool, isLast bool, result *vm.CallResult) error {
	...
	if ctr.joinBat == nil {
		ctr.joinBat, ctr.cfs = colexec.NewJoinBatch(ap.ctr.bat, proc.Mp())
	}
	...
	for i := ap.ctr.lastrow; i < count; i++ {
		...
		if err := colexec.SetJoinBatchValues(ctr.joinBat, ap.ctr.bat, int64(i),
		...
		if !matched && !nulls.Any(vec.GetNulls()) {
			for k, pos := range ap.Result {
				if err := ctr.rbat.Vecs[k].UnionOne(ap.ctr.bat.Vecs[pos], int64(i), proc.Mp()); err != nil {
					return err
				}
			}
			rowCountIncrease++
